...
/**
 * Whether the credential-seam notice has already been logged.
 *
 * The seam is genuinely unavailable on compositions that do not inject the
 * credentials service — the desktop shell is one — and that is not an error: the
 * credentials file below is the working fallback. Resolution happens per request,
 * so an unguarded warn printed one line every poll (measured: every ~25s in the
 * desktop shell). One line per process is honest; a running commentary is not.
 */
let credentialSeamNoted = false
...
export async function readApiKey(ctx) {
  ...
  try {
    ...
    if (!credentialSeamNoted) {
      credentialSeamNoted = true
      ctx.logger.warn(`balance-float: credential seam unavailable, using the credentials file instead (not logged again): ${error instanceof Error ? error.message : String(error)}`)
    }
  }
